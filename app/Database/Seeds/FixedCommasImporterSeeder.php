...
class FixedCommasImporterSeeder extends Seeder
{
    public function run()
    {
        $dir = ROOTPATH . '.agents/';
        ...
        $filePath = $files[0];
        echo "Clean Import with Number Format Fix: " . basename($filePath) . "\n";

        $db = \Config\Database::connect();

        // 1. ROLLBACK PREVIOUS IMPORTS
        $db->transStart();
        ...
        // Also delete Master Barang from this import to update their Gram weights correctly
        $db->query("DELETE FROM barang WHERE id NOT IN (SELECT DISTINCT id_barang FROM batch)");
        $db->transComplete();

        echo "Rolled back old import & unreferenced master barang.\n";
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $status = (stristr($catatan, 'Tidak Aman') !== false || stristr($catatan, 'Rusak') !== false) ? 'Tidak Aktif' : 'Aktif';
        ...
        if ($db->transStatus() === false) {
            ...
            echo "BERHASIL! $insertedCount batch stok telah diimpor dengan format Gram yang 100% presisi!\n";
        }
    }
