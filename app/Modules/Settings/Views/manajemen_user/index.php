...
<style>
    ...
    .wh-btn-primary {
        ...
        height: 42px; border-radius: 12px; font-size: 0.85rem; font-weight: 600;
        padding: 0 20px; display: inline-flex; align-items: center; gap: 7px;
    ...
    .wh-alert {
        ...
        font-size: 0.85rem; margin-bottom: 16px; display: flex; align-items: flex-start; gap: 10px;
    }
    ...
    .wh-dt-toolbar .dt-length-wrap select {
        ...
        border: 1px solid var(--wh-border); font-size: 0.82rem;
        padding: 0 8px; color: var(--wh-text);
    }
    .wh-dt-search {
        position: relative;
    }
    .wh-dt-search i {
        ...
        color: var(--wh-text-soft); font-size: 0.82rem;
    }
    .wh-dt-search input {
    ...
    /* Table */
    #tableUsers { border-collapse: separate; border-spacing: 0; width: 100%; }
    #tableUsers thead th {
        background: var(--wh-bg); color: var(--wh-text-soft);
        font-size: 0.68rem; font-weight: 600; text-transform: uppercase;
        letter-spacing: 0.04em; padding: 0 20px; height: 54px;
    ...
    #tableUsers, #tableUsers th, #tableUsers td { border-left: none; border-right: none; }
    #tableUsers tbody tr { transition: background 150ms ease; }
    #tableUsers tbody tr:hover { background: var(--wh-bg); }
    #tableUsers tbody tr:last-child td { border-bottom: none; }
    ...
    .wh-user-cell .email { font-size: 0.76rem; color: var(--wh-text-soft); margin-top: 1px; }
    ...
    .wh-badge.admin   { background: #EEF2FF; color: #4338CA; }
    .wh-badge.petugas { background: #ECFDF5; color: #047857; }
    .wh-badge.supervisor { background: var(--wh-warning-soft); color: #B45309; }
    .wh-badge.aktif   { background: #ECFDF5; color: #15803D; }
    .wh-badge.nonaktif{ background: var(--wh-danger-soft); color: #B91C1C; }

    /* Last login */
    .wh-login-date { font-size: 0.8rem; font-weight: 500; color: var(--wh-text); }
    .wh-login-time { font-size: 0.72rem; color: var(--wh-text-soft); margin-top: 1px; }
    .wh-login-null { color: var(--wh-text-soft); font-size: 0.82rem; }
    ...
    .wh-action-btn:active { transform: scale(0.98); }
    .wh-action-btn.edit   { background: var(--wh-primary-soft); color: var(--wh-primary); }
    .wh-action-btn.edit:hover { background: #DBEAFE; }
    .wh-action-btn.reset  { background: var(--wh-warning-soft); color: #B45309; }
    .wh-action-btn.reset:hover { background: #FDE68A; }
    .wh-action-btn.danger { background: var(--wh-danger-soft); color: #B91C1C; }
    .wh-action-btn.danger:hover { background: #FECACA; }
    .wh-action-btn.success{ background: var(--wh-success-soft); color: #047857; }
    .wh-action-btn.success:hover { background: #BBF7D0; }
    ...
    /* Modal overrides */
    .modal-content { border-radius: 16px; border: 1px solid var(--wh-border); }
    ...
    .modal-body .form-text { font-size: 0.75rem; color: var(--wh-text-soft); }
    .modal-body .alert {
        border-radius: 10px; font-size: 0.83rem;
        background: var(--wh-warning-soft); border-color: #FDE68A; color: #B45309;
    }
    ...
    .modal-footer .btn-warning {
        background: var(--wh-warning-soft); border-color: #FDE68A;
        color: #B45309; border-radius: 10px; font-size: 0.85rem; font-weight: 600;
        height: 42px; padding: 0 18px;
    }
                ...
                <tbody>
                    <?php $no = 1; foreach ($users as $row): ?>
                        <?php
                            ...
                            // Badge role
                            $roleBadge = 'petugas'; $roleLabel = esc($row['role_name']);
                            if ($row['role_name'] == 'Administrator') $roleBadge = 'admin';
                            elseif (stripos($row['role_name'], 'supervisor') !== false) $roleBadge = 'supervisor';
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['active']): ?>
                                    <span class="wh-badge aktif"><i class="fa-solid fa-circle" style="font-size:0.5rem;"></i> Aktif</span>
                                <?php else: ?>
                                    <span class="wh-badge nonaktif"><i class="fa-solid fa-circle" style="font-size:0.5rem;"></i> Nonaktif</span>
                                <?php endif; ?>
                            </td>
                            <td><?= $lastLogin ?></td>
                            <td>
                                <div class="wh-actions">
                                    <button class="wh-action-btn edit" title="Edit"
                                        data-bs-toggle="modal" data-bs-target="#modalEdit<?= $row['id'] ?>">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </button>
                                    <button class="wh-action-btn reset" title="Reset Password"
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <form action="<?= site_url('manajemen-user/reset/' . $row['id']) ?>" method="post" class="loading-form" data-overlay="true">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                            <button type="submit" class="btn btn-warning"><i class="fa-solid fa-key me-2"></i> Reset Password</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
