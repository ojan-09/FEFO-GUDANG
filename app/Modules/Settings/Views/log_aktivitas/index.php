...
<style>
    ...
    .timeline-card:hover .tl-time::after {
        background: var(--wh-primary);
        box-shadow: 0 0 0 1px var(--wh-primary);
    }
    ...
    /* =============================================
       DRAWER — Full Screen Fix
       Kompensasi html zoom: 0.75 + body margin-left sidebar
       ============================================= */
    .drawer-overlay {
        position: fixed;
        top: 0;
            ...
            <div class="col-md-2">
                ...
                <select name="role" class="form-select">
                    ...
                    <?php foreach ($roles as $r): ?>
                        <option value="<?= $r->name ?>" <?= ($filter_role == $r->name) ? 'selected' : '' ?>>
                            <?= ucfirst($r->name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2 d-flex gap-2">
                <button type="submit" class="wh-btn-primary w-100 justify-content-center">
                    <i class="fa-solid fa-filter"></i> Terapkan
                </button>
                <a href="<?= site_url('pengaturan/log-aktivitas') ?>" class="wh-btn-outline px-3" title="Reset Filter">
...
<div class="detail-drawer" id="detailDrawer">
    ...
    <div class="drawer-body">
        ...
        <div class="drawer-item mb-3">
            ...
            <div class="drawer-val fw-medium text-dark" id="drwSummary"
                style="background:#F8FAFC; padding:12px 14px; border-radius:10px; border:1px solid var(--wh-border); margin-top:4px;">
                -</div>
        </div>
        ...
        <div class="mt-4 pt-3 border-top">
            <button type="button"
                class="btn btn-sm btn-light w-100 text-start fw-semibold d-flex justify-content-between align-items-center"
                onclick="toggleAuditJson()"
                style="border: 1px solid #E2E8F0; padding: 10px 14px; border-radius: 8px;">
            ...
            <div id="auditJsonBody" style="display:none; margin-top:8px;">
                <pre id="drwRawJson"
                    style="background:#0f172a; color:#f8fafc; padding:12px; border-radius:8px;
        ...
        <div id="drwActionContainer" style="display:none; margin-top:24px;">
            <a href="#" id="drwActionBtn" class="drawer-action-btn">
                Lihat Halaman Transaksi <i class="fa-solid fa-arrow-right ms-2"></i>
            </a>
        </div>
    </div>
</div>
...
<script>
    function openDrawer(element) {
        ...
        if (trxNo !== '') {
            ...
            if      (trxNo.startsWith('BM-'))  btn.href = '<?= site_url('transaksi/barang-masuk') ?>';
            else if (trxNo.startsWith('BK-'))  btn.href = '<?= site_url('transaksi/barang-keluar') ?>';
            else if (trxNo.startsWith('ADJ-')) btn.href = '<?= site_url('transaksi/penyesuaian') ?>';
    ...
    $(document).ready(function () {
        document.body.appendChild(document.getElementById('drawerOverlay'));
        document.body.appendChild(document.getElementById('detailDrawer'));
