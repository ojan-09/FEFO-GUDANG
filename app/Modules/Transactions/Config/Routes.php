...
$routes->group('transaksi', ['namespace' => 'App\Modules\Transactions\Controllers', 'filter' => 'rbac:Administrator,Petugas Gudang'], function ($routes) {
    ...
    $routes->post('penyesuaian/delete/(:num)', 'PenyesuaianStok::delete/$1');
    ...
    $routes->get('barang-keluar/berita-acara-word/(:num)', 'BarangKeluar::downloadBeritaAcaraWord/$1');
...
$routes->group('transaksi', ['namespace' => 'App\Modules\Transactions\Controllers', 'filter' => 'rbac:Administrator,Petugas Gudang,Pimpinan'], function ($routes) {
    ...
    // === Monitoring Expired ===
    $routes->get('monitoring-expired', 'MonitoringExpired::index');
    $routes->post('monitoring-expired/ajaxData', 'MonitoringExpired::ajaxData');
    $routes->get('monitoring-expired/detail/(:num)', 'MonitoringExpired::detail/$1');
    $routes->get('monitoring-expired/export/excel', 'MonitoringExpired::exportExcel');
    $routes->get('monitoring-expired/export/pdf', 'MonitoringExpired::exportPDF');
});
